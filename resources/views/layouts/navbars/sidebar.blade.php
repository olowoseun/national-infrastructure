<div class="sidebar" data-color="rose" data-background-color="RED" data-image="{{ asset('material') }}/img/sidebar-1.jpg">
  <!--
        ...
        <div class="collapse {{ ($menuParent == 'dashboard' || $menuParent == 'laravel') ? ' show' : '' }}" id="laravelExample">
          <ul class="nav">            
            ...
            @can('manage-items', App\User::class)
              <li class="nav-item{{ $activePage == 'asset-management' ? ' active' : '' }}">
                <a class="nav-link" href="{{ route('asset.index') }}">
                  <span class="sidebar-mini"> AM </span>
                  <span class="sidebar-normal"> {{ __('Asset Management') }} </span>
                </a>
              </li>
            @endcan
            {{-- @can('manage-items', App\User::class)
